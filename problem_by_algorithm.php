...
<div class="my_right_content">
 ...
 $distAlg = "SELECT * From probleminfo GROUP BY problemCategory HAVING COUNT(*) >=1";
 $result = mysql_query($distAlg);
 
 while($row=mysql_fetch_array($result)){
    $probCategory =  $row['problemCategory'];
	
	echo '<br/><br/><span style ="font-family:comic sans ms;font-size:20pt;color:#0066ff;">'.$probCategory . '</span><hr>';
	//echo $probCategory.'<br/>';
	
	$myQ = "SELECT * FROM probleminfo WHERE problemCategory = '$probCategory'";
	$res = mysql_query($myQ);
