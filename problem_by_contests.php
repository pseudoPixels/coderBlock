  ...
  while($row=mysql_fetch_array($result)) {
	...
	//this date1 contains the current date and time
		...
		if($secVal >= 0 && $secVal > $compDuration){
		 // echo "<span style='font-family:comic sans ms; font-size:15pt;color:33CC00;'>".$compTitle."</span><br/>";
		  echo '<br/><br/><span style ="font-family:comic sans ms;font-size:20pt;color:#0066ff;">'.$compTitle . '</span><hr>';
		  
		  $probSql = "SELECT * FROM probleminfo WHERE contestID = '$compID'";
		  $probResult = mysql_query($probSql);
		  
		  while($probRow= mysql_fetch_array($probResult)){
		      $probTitle = $probRow["problemTitle"];
			  $pID = $probRow["problemId"];
		    //echo $probTitle."<br/>";
			
			echo "<a href='viewProblemGivenID.php?id="."$pID'"."><span style ='font-family:comic sans ms;font-size:15pt;color:#006600;'><strong>icb".$probRow['problemId']. "</strong>: &nbsp &nbsp ".$probTitle."</span></a> &nbsp &nbsp";
	       ...
	       echo "<a href='login_success.php?problemID="."$pID'"."><span style ='font-family:comic sans ms;font-size:10pt;color:#FF0000;'> <strong> Submit </strong></span></a><br/>";
			...
			//echo "<a href='browseProblem.php?id="."$probID' target='_blank'"."><span style='color:green;'>$probTitle</span></a><br/>";
