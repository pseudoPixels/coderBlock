...
<span style ="font-size:18pt;color:#000000;">Team Registration Info</span><hr><br/>
<?php
          $teamID = $_POST['teamID'];
		  ...
		  $contestID = $_GET['contestID'];
		   ...
		   if($count==1){
		      $sql="SELECT * FROM teaminfo WHERE teamID='$teamID' and teamPassword='$teamPassword' and memberId01='$username'";
		      $res=mysql_query($sql);
			  $c = mysql_num_rows($res);
			  if($c==1){
			     
				 $checkRegSql = "SELECT * FROM contestteamlist WHERE teamID='$teamID' and contestID='$contestID'";
			     $queryRes = mysql_query($checkRegSql);
				 $numOfRowRet = mysql_num_rows($queryRes);
				 if($numOfRowRet>=1){
				   echo '<span style="color:red; font-size:25px;">Team is already registered !!!</span>';
				 }else{
				   $insQ = "INSERT INTO contestteamlist(teamID, contestID) VALUES('$teamID', '$contestID')";
				   $insRes = mysql_query($insQ);
				   echo '<span style="color:green; font-size:25px;">Team registration completed successfully</span>';
				 }
			   
			  }else{
			    echo '<span style="color:red; font-size:25px;">Sorry, Only Team Leader can register a team. You are not the team leader of specefied team.</span>';
			  }
