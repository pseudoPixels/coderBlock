...
<?php
          $teamID = $_POST['joiningTeamName'];
		  $teamPassword = $_POST['joingTeamPassword'];
		   ...
		   if($count==1){
		      if($row['numberOfTeamMembers']<3){
			     $username = $_SESSION['userName'];
			     $sql2 = "SELECT * FROM teaminfo WHERE teamID='$teamID' and teamPassword='$teamPassword' and ( memberId01 = '$username' or memberId02 = '$username' or  memberId03 ='$username' )";
			     $res = mysql_query($sql2);
				 $r = mysql_fetch_array($res);
				 $c = mysql_num_rows($res);
				 if($c==0){
				    $numberOfMembers = $row['numberOfTeamMembers'];
					
					if($numberOfMembers == 1){
					   $updateQuery = "UPDATE teaminfo SET memberId02 = '$username' , numberOfTeamMembers = 2 WHERE teamID = '$teamID'";
					   $upRes = mysql_query($updateQuery);
					   
					}else{
					   $updateQuery = "UPDATE teaminfo SET memberId03 = '$username' , numberOfTeamMembers = 3 WHERE teamID = '$teamID'";
					   $upRes = mysql_query($updateQuery);
					}
					
					echo '<br/><br/><br/><span style="color:green; font-size:25px;"> You have been added as a member of the team </span>';
					
				   
				   
				 }else{
				   echo '<br/><br/><br/><span style="color:red; font-size:25px;">You are already a member of the Team !!! </span>';
				 }
			  
			  }else{
			    echo '<br/><br/><br/><span style="color:red; font-size:25px;">Sorry, the team already contains 3 Memebers. Please, Try another team or contact with team leader .</span>';
			  }
			  
		   }
		   else {
		      echo '<br/><br/><br/><span style="color:red; font-size:25px;">Sorry, The team you mentioned does not exists !!!</span>';
		   }
?>
